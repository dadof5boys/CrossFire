...
- **Chat / play**: Socket.IO on the Fastify process (ephemeral in-memory state). Vite must proxy `/socket.io` with `ws: true`. `/play/:tableId` is the digital tabletop. The server enforces turn order, zone-by-`typeId` (realms = 13, pool = champions), and battlefield combat: `play:attack` opens a pending attack; the defender `play:defend`s with a pool champion or `play:decline-defend`. It looks up cards from `packages/data-pipeline/data/cards.json` — do not trust client-sent type/bonus. Draw/move/pass are blocked until the attack resolves.
