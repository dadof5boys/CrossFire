...
- **Chat / play**: Socket.IO on the Fastify process (ephemeral in-memory state). Vite must proxy `/socket.io` with `ws: true`. `/play/:tableId` is the digital tabletop. The server enforces turn order, zone-by-`typeId` (realms = 13, pool = champions), and a first-slice realm attack (`play:attack`). It looks up cards from `packages/data-pipeline/data/cards.json` — do not trust client-sent type/bonus.
